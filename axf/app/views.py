...
from .models import Wheel,Nav,Mustbuy,Shop,\
    MainShow,FoodTypes,Goods,User,Cart,Order
...
def cart(request):
    ...
    return render(request,'cart.html',{'title':'购物车','cartslist':cartslist})
...
def market(request,zid,cid,sortid):
    leftSlider = FoodTypes.objects.all()
    if cid == '0':
        productList = Goods.objects.filter(categoryid=zid)
    else:
        productList = Goods.objects.filter(categoryid=zid,childcid=cid)
    ...
    if sortid == '1':
        ...
        productList = productList.order_by('specifics')
    elif sortid == '3':
        productList = productList.order_by('-specifics')


    group = leftSlider.get(typeid=zid)
    childList = []
    ...
    return render(request,'market.html',content)
...
def login(request):
    if request.method == "POST":
        ...
        if f.is_valid():
            ...
            return render(request, 'login.html', {"title": "登陆", "form": f,"error":f.errors})
    else:
        f = LoginForm()
        return render(request, 'login.html', {"title": "登陆","form":f})
